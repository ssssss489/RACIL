...
class Prototype(torch.nn.Module):
    ...
    def multi_eig_M(self, x, y):
        if self.M_eig is None:
            ...
            self.M_inv = torch.linalg.inv(sym_M).float()
    ...
    def inv_mu_feature(self, x):
        return torch.matmul(x, self.M_inv)
...
class prototype_decoder(base_model):
        ...
        if model_type == 'mlp':
            ...
            for _ in range(self.n_tasks):
                self.Prototypes.append(
                    Prototype(args.top_n_eigs, 10, parameters[self.data_name].classifier[0]))
                self.Decoders.append(ResNet18_Decoder(parameters[self.data_name].classifier[0],
    ...
    def learn_step(self, inputs, labels, get_class_offset, task_p):
        ...
        decoder_outputs = decoder(prototype_feature)
        ...
        # for debug
        decoder_outputs_numpy = decoder_outputs.data.cpu().numpy()
    ...
    def train_step(self, inputs, labels, get_class_offset, task_p):
        ...
        if task_p > 0:
            ...
            with torch.no_grad():
                pt_inputs = pt_decoder(pt_feature)
...
class prototype_decoder_train(base_train):
    def __init__(self, data_loader, model, args, logger):
        ...
        self.train_process = [self.train_step] * (self.n_epochs - self.n_epochs_learn - 2) \
                            +[self.learn_step] * self.n_epochs_learn \
                            +[lambda : self.compute_prototype('mu')] \
                            +[lambda : self.compute_prototype('cov')]

    # def train_step(self):
    #     classifier_losses, decoder_losses, train_acc = [], [], []
    ...
    #     bar.close()
    #     print(f'    classifier_loss = {np.mean(classifier_losses)}, decoder_loss = {np.mean(decoder_losses)}, train_acc = {np.mean(train_acc)}')
